...
module.exports = function Spider(name) {
    ...
    function inQueue() {
        ...
        // console.log(private.name + ' queue length:'+private.worker.queue.length());
        
        if ((2 * private.worker.maxOccurs) < private.worker.queue.length()) {
            // console.log(private.name + ' maxOccurs!');
            return;
        }
        ...
        private.url.pops(private.url.list, private.worker.maxOccurs * 2).then(function (urls) {
           ...
           // console.log(name + ' count:' + private.worker.count);
